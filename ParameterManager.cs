    ...
    public class ParameterManager
    {
        ...
        public void ExportParameters(Document doc, string targetFolder)
        {
            ...
            try
            {
                ...
                // Get the folder name to use as the namespace
                string folderName = new DirectoryInfo(targetFolder).Name;

                // Generate the parameter file content
                string paramContent = GenerateParameterFileContent(doc, folderName);
                ...
                {
                    if (line.TrimStart().StartsWith("Public") && count < 5)
                    {
        ...
        /// <summary>
        /// Generates VB code defining a namespace with native-typed parameter properties.
        /// </summary>
        /// <param name="doc">The Inventor document</param>
        /// <param name="namespaceName">The namespace name to use (based on folder name)</param>
        /// <returns>VB code defining a namespace with parameter properties</returns>
        private string GenerateParameterFileContent(Document doc, string namespaceName)
        {
            StringBuilder sb = new StringBuilder();

            // Add a header comment
            sb.AppendLine("' Parameter file automatically generated by iLogic Parameter Manager");
                ...
                if (parameters != null)
                {
                    ...
                    foreach (Parameter param in parameters.UserParameters)
                    {
                        ...
                        try
                        {
                            ...
                            // Format the property definition directly in the namespace
                            // Example: Public Width As Double = 10.0
                            sb.AppendLine($"    Public {paramName} As {vbType} = {vbValue}");
                        }
                        catch (Exception ex)
                        {
